...
| 数据类型 | 存储方案 | 理由 |
|----------|----------|------|
| 行情/因子/信号矩阵 | Parquet（按日期分区） | 列式存储，压缩比高，支持 PIT 分区读取，无需数据库服务 |
...
**不引入独立数据库服务**（SQLite / TimescaleDB / InfluxDB / PostgreSQL）的阶段性原因：
- 单机研究阶段运维复杂度不必要；
- Parquet + NDJSON 已满足 PIT、可审计、可重放要求；
- 若日后扩展到多机/实盘执行，可在 G1 数据治理层增加数据库适配，不影响上层契约。
...
## 5. Python–Rust IPC 方案（Inter-Process Communication）

### 5.1 选定方案：Rust CLI 以子进程调用 Python 模块
...
**Rust CLI 行为**：
1. 解析命令行参数（clap）。
2. 通过 `std::process::Command` 启动 Python 子进程，传入结构化参数（JSON 或命令行参数）。
3. 捕获 stdout，校验 JSON schema（`CLI_OUTPUT_SCHEMA.json`），输出或透传给调用方。
4. 异常时捕获 stderr，映射为标准错误码后包装输出。

**备选方案（未采纳）及排除原因**：

| 方案 | 排除原因 |
|------|----------|
| gRPC / REST | 引入网络栈与服务部署复杂度，过度工程 |
...
Python quant core 暴露命令行入口：

```bash
python -m hiveflow.cli <subcommand> [--as-of ...] [--run-id ...] ...
```

输出统一为 JSON，写入 stdout，错误写入 stderr（不混入 stdout）。
...
| 指标 | 当前参考预算 | 触发复评条件 |
|------|-------------|-------------|
| 单次 CLI 调用延迟（P95） | ≤ 2s（含 Python 子进程启动） | 连续 3 个版本 P95 > 5s |
...
2. Rust CLI 优先实现：
   ...
   - 调用 Python 子进程并透传 stdout
3. 将 `make validate-cli-output` 纳入 CI 必过项。
4. 不引入数据库服务，Parquet + NDJSON 满足当前阶段需求。
