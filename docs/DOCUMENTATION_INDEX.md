...
| 量化评估使用方 | [FACTOR_OPTIMIZATION_P1_USAGE.md](FACTOR_OPTIMIZATION_P1_USAGE.md)（P1 使用与字段说明） | [AI_SKILLS_INTEGRATION.md](AI_SKILLS_INTEGRATION.md)（审批边界） | [CLI_OUTPUT_SCHEMA.json](CLI_OUTPUT_SCHEMA.json)（合同字段） |
...
`ARCHITECTURE.md` 定义系统分层与治理边界 -> `AI_SKILLS_INTEGRATION.md` 定义 AI 协作与审批边界 -> `TECH_STACK_DECISION.md` 固化技术选型与复评机制 -> `CLI_OUTPUT_SCHEMA.json` 定义机器可校验输出合同 -> `CLI_OUTPUT_EXAMPLES.md` 提供样例与校验入口 -> `FACTOR_OPTIMIZATION_P1_USAGE.md` 提供 P1 因子优化实操指南。
