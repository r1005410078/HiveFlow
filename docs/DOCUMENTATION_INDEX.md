...
| 角色 | 第一步 | 第二步 | 第三步 |
|---|---|---|---|
| 策略研发 | [ARCHITECTURE.md](ARCHITECTURE.md)（分层职责、上线闸门） | [AI_SKILLS_INTEGRATION.md](AI_SKILLS_INTEGRATION.md)（AI 边界、审批边界） | [CLI_OUTPUT_EXAMPLES.md](CLI_OUTPUT_EXAMPLES.md)（输出样例） |
...
| 平台工程 | [ARCHITECTURE.md](ARCHITECTURE.md)（G1/G2/G3、全链路契约） | [AI_SKILLS_INTEGRATION.md](AI_SKILLS_INTEGRATION.md)（运行流程、权限模型） | [CLI_OUTPUT_SCHEMA.json](CLI_OUTPUT_SCHEMA.json)（审计字段、接入解析） |
...
3. [CLI_OUTPUT_SCHEMA.json](CLI_OUTPUT_SCHEMA.json)
4. [CLI_OUTPUT_EXAMPLES.md](CLI_OUTPUT_EXAMPLES.md)
...
`ARCHITECTURE.md` 定义系统分层与治理边界 -> `AI_SKILLS_INTEGRATION.md` 定义 AI 协作与审批边界 -> `CLI_OUTPUT_SCHEMA.json` 定义机器可校验输出合同 -> `CLI_OUTPUT_EXAMPLES.md` 提供样例与校验入口。
