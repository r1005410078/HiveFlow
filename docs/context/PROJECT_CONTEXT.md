# PROJECT_CONTEXT

## 项目名

- 英文：HiveFlow
- 中文：策略流（沿用历史语境）

## 一句话定位

一个由多策略协作驱动的个人资产决策系统，用于把策略、持仓、风险、目标持仓和调仓建议放在同一个可追溯系统中。

## 核心目标

- 不是”问 AI 买什么”
- 而是建立”可信状态 + 可追溯决策 + 可执行动作”的闭环

## 系统宗旨（每次头脑风暴必读）

> HiveFlow 是一个**为 AI Agent 提供高质量上下文的投资工具系统**。

### 系统职责（HiveFlow 做的事）

- 记录和管理用户数据（持仓、目标配比、回测结果、风险信号、策略运行记录）
- 提供确定性工具（同步、回测、量化策略计算、调仓建议、交易执行）
- 通过 `--output json` 向 Agent 输出结构化、准确、完整的上下文
- 输出风险相关**数据与信号**（回撤、波动率、相关性、偏离、信号状态），不输出投资结论

### 系统边界（HiveFlow 不做的事）

- **不做投资判断**，不做智能分析
- **不做智能调用**，不决定”什么时候用哪个策略”
- **不做推荐输出**（不推荐风格、不推荐买卖动作），只输出可追溯数据
- **不输出部分降级结果作为决策上下文**（严格模式下缺失即失败）

### 智能工作流层（Skill 做的事）

- 分析 HiveFlow 输出的上下文（持仓、风险、回测结果、策略信号）
- 做投资判断（该不该调仓、选哪个策略、风险是否可接受）
- 把判断组织成可复用工作流（Skill）

### 外部运行时（Agent Runtime 做的事）

- 读取用户问题
- 加载和执行 Skill
- 调用 HiveFlow CLI
- 读取 JSON 输出并组织回复
- 在需要时请求用户确认

> 术语统一：系统做“风险数据输出”，Skill 做“决策流程编排”，Agent Runtime 做“外部执行与交互”。

### 分工原则

| 层 | 职责 | 技术形式 |
|---|---|---|
| HiveFlow CLI | 数据 + 工具（确定性）| Python CLI，可测试 |
| 量化策略 | 数学工具，输出信号/权重 | `BaseStrategy` 子类 |
| Skill | 决策流程 + 判断框架 | Markdown 工作流 |
| Agent Runtime | 执行 Skill，调用 CLI，与用户交互 | Codex / Claude / 其他 AI 平台 |

### 项目内 / 项目外边界

- 项目内：`HiveFlow` + `Skill`
- 项目外：`Agent Runtime`

结论：
- 你不需要自己实现 Agent
- 你需要维护的是稳定的 Tool Layer 和 Workflow Layer

### 当前推荐的智能工作流

- `hiveflow-daily-check`：先做每日检查，判断是否需要进一步动作
- `hiveflow-strategy-selector`：需要时比较策略、决定是否切换
- `hiveflow-rebalance-executor`：在目标明确后生成候选动作并等待确认
- `hiveflow-portfolio-advisor`：作为总控 Skill，负责在多阶段任务中调度上述子 Skill

### 判断一个功能是否该加的问题

> 这个功能是**确定性工具**（给定输入总是输出同样结果），还是需要**判断**？
> - 是工具 → 可以加进 HiveFlow
> - 需要判断 → 放进 Skill，HiveFlow 只提供数据接口

## 核心对象

- 策略（Strategy）
- 策略席位（Slot）
- 当前策略（Current Strategy）
- 实际持仓（Positions）
- 目标持仓（Target Allocations）
- 风险信号（Risk Signals）
- 调仓建议（Rebalance Suggestions）
- 决策日志（Decision Logs）

## 建模原则（当前共识）

- 策略信息至少分三层：
  - 策略名称（name）
  - 策略类型（strategy_type，底层兼容 category）
  - 策略维度（dimension）
- “策略权重”和“当前策略”不是一回事：
  - 策略权重：组合结构
  - 当前策略：当前时点的执行主策略

## 开发原则

- CLI 优先，把业务闭环做实
- 先可追溯，再智能化
- 每个 chunk 都要有测试与文档回填
