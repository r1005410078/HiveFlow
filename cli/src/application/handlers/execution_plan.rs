...
use crate::infrastructure::table_renderer::render_execution_plan_table;
use serde_json::Value;

const PHASE1_NOTIONAL: f64 = 1_000_000.0;

pub fn handle(args: ExecutionPlanRequest) -> Result<(), AppError> {
    ...
    let pre_full = http_client::post_pretrade_check(
        &cfg.server_url,
        &args.as_of,
    ...
    let pretrade_data = pre_full.get("data").cloned();

    let out = http_client::post_execution_plan(
        &cfg.server_url,
        &args.as_of,
        target_weights,
        pretrade_data,
    ...
    match args.output.as_str() {
        "json" => print!("{out}"),
        "table" => {
            let table = render_execution_plan_table(&out);
            print!("{table}");
        }
        other => {
            return Err(AppError::InvalidArgs(format!(
                "unsupported --output value for execution plan: {other} (expected: json|table)"
            )));
        }
    }
    Ok(())
